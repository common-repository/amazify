...
function amazify_settings_init(){ 
	...
	add_settings_field( 
		'amazify_catchall', 
		__( 'Catch all URL', 'amazify' ), 
		'amazify_catchall_render', 
		'Amazify', 
		'amazify_Amazify_section' 
	);
...
function amazify_catchall_render(){ 
	$options = get_option( 'amazify_settings' );
	?>
	<input type='text' name='amazify_settings[amazify_catchall]' value='<?php echo esc_attr($options['amazify_catchall']); ?>'>
...
function AddTag($content){
	...
	$catchall_option = $options_amazify['amazify_catchall'];
	
	if(isset($options_amazify) && $options_amazify['amazify_tag'] != ""){
